...
\item Bagaimana distribusi potensial dan medan listrik pada kapasitor tabung tegangan tinggi sistem \textit{flash}, termasuk efek tepi yang memicu risiko dadal listrik (Kasus 2)?
\item Bagaimana tetesan kristal cair (PDLC) pada \textit{ND filter} elektronik menata ulang medan listrik penggerak seragam di sekitarnya (Kasus 3)?
...
\item Menyelesaikan persamaan Laplace silindris kapasitor tabung \textit{flash} dan memvalidasinya terhadap solusi koaksial analitik (Kasus 2).
\item Menghitung medan listrik tereduksi di dalam tetesan kristal cair PDLC dan memvalidasinya terhadap teori bola dielektrik (Kasus 3).
...
\vspace{2pt}
\noindent{\small Catatan: \textit{source code} lengkap Kasus 1 dapat diakses pada repositori \href{https://github.com/SandyFauzi/Fisika-Komputasi-5}{\texttt{github.com/SandyFauzi/Fisika-Komputasi-5}}.}

% ============================================================
\section{Kasus 2: Distribusi Potensial dan Medan Listrik Kapasitor Tabung Tegangan Tinggi pada Sistem \textit{Flash} (Laplace Silindris)}
% ============================================================

\subsection{Pendahuluan}

\subsubsection{Latar Belakang}
Sistem lampu kilat (\textit{flash}) pada kamera, terutama unit strobo eksternal, memerlukan kapasitor penyimpan energi tegangan tinggi untuk melepaskan denyut daya besar ke tabung \textit{xenon} dalam waktu sangat singkat. Komponen ini umumnya berupa kapasitor tabung elektrolitik (\textit{tubular electrolytic capacitor}) yang tersusun atas gulungan foil elektroda dalam (anoda) berjari-jari $a$ yang terbungkus rapat di dalam selongsong logam (\textit{casing}) sebagai elektroda luar berjari-jari $b$ dan \textit{grounded}. Konfigurasi ini secara geometris setara dengan kapasitor koaksial dengan celah anular (\textit{annular gap}) berisi dielektrik di antara kedua elektroda, dan diisi hingga tegangan ekstrem orde ratusan volt ($V_0 \approx 330$~V).

Karena gulungan foil memiliki panjang berhingga (\textit{finite}), garis medan di kedua ujung tabung tidak lagi murni radial sebagaimana pada kapasitor koaksial ideal yang panjangnya tak hingga, melainkan melengkung keluar menembus dielektrik dan udara di sekitarnya. Gejala ini dikenal sebagai efek tepi atau \textit{fringing field}, dan tidak dapat dijelaskan oleh rumus kapasitor satu dimensi. Pemetaan medan tepi ini bersifat kritis secara rancangan: di ujung gulungan, konsentrasi medan listrik dapat memicu kegagalan dielektrik (\textit{dielectric breakdown}) berupa loncatan bunga api ke \textit{casing}, sehingga diperlukan untuk menentukan ketebalan isolator ujung yang aman. Untuk memerikan medan secara lengkap, diperlukan penyelesaian dua dimensi pada bidang penampang $r$-$z$. Dalam keadaan bebas muatan ruang (\textit{space charge}), potensial memenuhi persamaan Laplace, yang di sini diselesaikan secara numerik melalui metode beda hingga statik (\textit{Finite-Difference Frequency-Domain}, FDFD).

\subsubsection{Rumusan Masalah}
Permasalahan yang dikaji dapat dirumuskan sebagai berikut: (1) bagaimana distribusi potensial listrik pada celah koaksial antara gulungan elektroda dalam dan \textit{casing}; (2) bagaimana arah dan besar medan listrik $\mathbf{E}$, termasuk manifestasi efek tepi yang berpotensi memicu dadal listrik di ujung gulungan; serta (3) seberapa akurat solusi numerik FDFD jika ditakar terhadap rumus kapasitor koaksial analitik pada irisan yang jauh dari ujung.

\subsubsection{Tujuan}
Penelitian ini bertujuan memodelkan celah koaksial elektroda dalam--\textit{casing} kapasitor \textit{flash} secara elektrostatik, menyelesaikan persamaan Laplace koordinat silinder dengan metode FDFD, memvisualisasikan peta potensial beserta vektor arah medan listrik, dan menganalisis efek tepi secara kuantitatif sebagai dasar penentuan ketebalan isolator ujung.
...
Secara fisis, Persamaan Laplace bermakna bahwa potensial di titik mana pun merupakan nilai rata-rata persis dari potensial di sekelilingnya (sifat harmonik). Tidak ada puncak atau lembah potensial lokal di wilayah dielektrik; titik ekstrem selalu berada di batas konduktor (gulungan elektroda dalam dan \textit{casing}).

\subsubsection{Persamaan Laplace Koordinat Silinder}
Mengingat gulungan kapasitor tabung memiliki simetri perputaran terhadap sumbu tabung ($z$), kita meninjau masalah pada sistem koordinat silinder $(r, \theta, z)$. Karena sistem simetris secara azimutal (potensial tidak bergantung sudut putar $\theta$), turunan $\partial / \partial \theta$ bernilai nol. Penjabaran operator Laplacian $\nabla^2$ dalam koordinat silinder berdimensi dua $r-z$ menjadi:
...
Jika kita mengabaikan ujung tabung (menganggap panjang gulungan tidak terhingga), maka persoalan menjadi 1D radial belaka. Integrasi ganda $\frac{1}{r}\frac{\partial}{\partial r}\left(r\frac{\partial V}{\partial r}\right) = 0$ menghasilkan solusi logaritmik $\ln(r)$. Menerapkan batas tegangan $V(a) = V_0$ pada elektroda dalam, dan $V(b) = 0$ (\textit{ground}) pada \textit{casing} luar, menghasilkan solusi teoretis murni (Kapasitor Koaksial Silindris):
...
Domain komputasi berbentuk persegi panjang pada bidang $r$-$z$ dengan jari-jari elektroda dalam $a = 4$~mm, jari-jari \textit{casing} $b = 10$~mm, dan panjang aksial $L_z = 30$~mm. Gulungan elektroda dalam diberi potensial $V_0 = 330$~V dan sengaja dibuat lebih pendek daripada domain agar efek tepi terpapar di kedua ujungnya. Domain didiskritisasi memakai kisi (\textit{grid}) seragam berukuran $70 \times 150$ titik, dengan langkah ruang $\Delta r$ dan $\Delta z$ yang seragam.
...
\end{equation}
Pada sumbu $r = 0$ diberlakukan syarat regularitas (\textit{regularity condition}) berupa $\partial V/\partial r = 0$ untuk menghindari singularitas suku $\frac{1}{r}\frac{\partial V}{\partial r}$, sedangkan permukaan gulungan elektroda dalam, \textit{casing}, dan batas domain memakai syarat Dirichlet. Seluruh titik kisi dirakit menjadi sistem persamaan linear \textit{sparse} . Sistem ini diselesaikan secara langsung (\textit{direct solver}) melalui dekomposisi LU \textit{sparse} (\textit{SuperLU}) yang dipanggil melalui fungsi spsolve, lalu komponen medan listrik dihitung dengan skema beda tengah (\textit{central difference}).

\subsubsection{Algoritma dan Akselerasi Komputasi}
Perakitan matriks (\textit{matrix assembly}) yang memuat banyak elemen titik batas dapat berjalan lambat jika dieksekusi dengan perulangan standar Python, terutama ketika struktur matriks perlu diperbarui untuk setiap variasi posisi dudukan gulungan foil terhadap \textit{casing}. Untuk mengoptimalkan waktu komputasi, fungsi pembentukan elemen dieksekusi bersama fungsi \textit{Just-In-Time} (JIT) melalui paket modul \textit{Numba} (\texttt{@njit}). \textit{Numba} mereduksi latensi interpretasi operasional bawaan Python menjadi instruksi level perangkat keras (\textit{machine code}). Matriks jarang (\textit{sparse matrix}) dimensi besar ($A \cdot x = b$) ini kemudian dikomputasi menggunakan subrutin dekomposisi \textit{LU} (SuperLU) milik SciPy. Eksekusi JIT \textit{Numba} dan metode pencari invers langsung \textit{SuperLU} mempercepat iterasi penyelesaian secara keseluruhan tanpa perlu perhitungan metode tebak-ralat iteratif.

\subsection{Hasil dan Pembahasan}

Berdasarkan komputasi pada Gambar \ref{fig:kasus2}, distribusi fluks tergambar melalui gradien pemetaan nilai skalar beda potensial ($V$) dan panah vektor medan listrik ($E$). Di rentang silinder tengah gulungan elektroda dalam, garis medan terdistribusi merata dengan arah tegak lurus terhadap permukaan elektroda. Arah orientasi panah melukiskan lintasan medan radial dari elektroda bertegangan tinggi menuju \textit{casing} yang \textit{grounded} (area potensial rendah).

Pada rentang posisi ujung gulungan, tampak garis medan listrik melengkung (deviasi radial asimetris). Kelengkungan medan ekuipotensial eksterior (\textit{fringing field}) terjadi akibat fakta bahwa elektroda foil bukan bersifat memanjang tak terhingga seperti pada kapasitor koaksial ideal. Justru di kawasan inilah konsentrasi medan tertinggi muncul sehingga rawan memicu dadal listrik (\textit{breakdown}) ke \textit{casing}; pemetaan dua dimensi ini menjadi dasar kuantitatif untuk menentukan margin ketebalan isolator ujung. Fenomena fisis dua dimensi ini dapat disimulasikan menggunakan matriks \textit{sparse} stensil batas fisis, yang merangkum variasi nilai ruang di setiap piksel untuk merekam "kebocoran" arah orientasi tanpa batasan aproksimasi komputasi yang rumit.

Untuk validasi, ditunjukkan profil perbandingan gradien skalar lintang batas $V(r)$ tengah silinder. Perbandingan garis data komputasi ekuivalen (titik dot) dan kurva acuan logaritma kapasitor koaksial analitik (putus-putus) menunjukkan akurasi resolusi simulasi numerikal dalam metode diskritisasi yang cukup presisi, mencatatkan defisiasi toleransi \textit{error} yang kecil $\approx 2,37\%$. Faktor persentase kecil tersebut berasal dari limitasi panjang matriks kisi ruang ($\Delta r$) serta pelebaran yang kecil dari deviasi (\textit{fringing}) ke arah sentral geometri. Hal ini membuktikan kemampuan FDFD metode konservatif bagi pemodelan elektrostatika kapasitor tabung nyata.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/kasus2.png}
\caption{\textit{Output} program Kasus 2: elektrostatik kapasitor tabung \textit{flash} elektroda dalam--\textit{casing} (potensial, besar dan arah medan, serta validasi koaksial)}
\label{fig:kasus2}
\end{figure}

\vspace{2pt}
\noindent{\small Catatan: \textit{source code} lengkap Kasus 2 dapat diakses pada repositori \href{https://github.com/SandyFauzi/Fisika-Komputasi-5}{\texttt{github.com/SandyFauzi/Fisika-Komputasi-5}}.}

% ============================================================
\newpage
\section{Kasus 3: Distribusi Medan Listrik pada Tetesan Kristal Cair (PDLC) \textit{ND Filter} Elektronik dalam Medan Seragam (Volume Hingga)}
% ============================================================

\subsection{Pendahuluan}

\subsubsection{Latar Belakang}
Kamera sinema profesional kini dilengkapi fitur \textit{Electronic Variable ND Filter}, yaitu kaca penapis yang kerapatannya (kemampuan menggelap) dapat diatur secara elektrik tanpa mengganti filter fisik. Salah satu teknologi yang mendasarinya adalah \textit{Polymer Dispersed Liquid Crystal} (PDLC): pada skala mikroskopis, material kristal cair tersebar sebagai tetesan-tetesan (\textit{droplet}) dielektrik berbentuk hampir bola di dalam matriks polimer. Ketika tegangan dikenakan melalui sepasang elektroda transparan ITO, terbentuk medan listrik penggerak yang hampir seragam $E_0$ menembus tetesan kristal cair, memutar orientasi molekulnya sehingga indeks bias berubah dan cahaya yang lewat teratenuasi.

Pada keadaan ini, tiap tetesan kristal cair berperilaku sebagai bola dielektrik berjari-jari $a$ (orde mikrometer) yang direndam di dalam medan seragam. Tetesan terpolarisasi (\textit{polarization}) dan membangkitkan muatan terikat (\textit{bound charge}) pada permukaannya, yang menata ulang sebaran medan di dalam dan di sekitar tetesan. Karena tidak terdapat muatan bebas, potensial tetap memenuhi persamaan Laplace, tetapi dengan lompatan permitivitas (\textit{permittivity jump}) pada antarmuka tetesan--matriks. Penanganan lompatan ini menuntut formulasi yang menjaga kontinuitas komponen normal medan perpindahan listrik (\textit{displacement field}) $\mathbf{D}$. Geometri bola tidak memiliki solusi analitik tertutup untuk konfigurasi sembarang, sehingga persoalan diselesaikan secara numerik dengan metode volume hingga (\textit{finite volume method}, FVM).

\subsubsection{Rumusan Masalah}
Permasalahan yang dikaji: (1) bagaimana distribusi medan listrik di dalam dan di sekitar tetesan kristal cair yang ditempatkan dalam medan penggerak seragam; (2) bagaimana perilaku garis medan saat melintasi antarmuka dielektrik tetesan--matriks; serta (3) seberapa besar atenuasi medan di bagian dalam tetesan dibandingkan dengan prediksi analitik bola dielektrik klasik.

\subsubsection{Tujuan}
Penelitian bertujuan menyelesaikan persamaan Laplace koordinat bola dengan syarat batas dielektrik menggunakan metode volume hingga, memvisualisasikan besar dan arah medan listrik pada tetesan kristal cair PDLC, dan memvalidasi medan interior tetesan terhadap hasil klasik bola dielektrik dalam medan seragam.

\subsection{Teori Dasar}

\subsubsection{Elektrostatika Medium Dielektrik dan Polarisasi}
Berbeda dengan bahan konduktor yang membiarkan muatan bergerak bebas ke permukaan, material dielektrik seperti kristal cair pada tetesan PDLC terbuat dari molekul-molekul yang menahan erat elektronnya. Ketika tetesan diletakkan di tengah medan listrik penggerak $\mathbf{E}_{0}$, molekul material merespon dengan memelarkan dan menjajarkan posisi awan elektronnya relatif terhadap inti (polarisasi). Ini menciptakan momen dipol terinduksi berskala makroskopik yang direpresentasikan dengan rapat polarisasi $\mathbf{P}$.
...
Perhatikan bahwa konstanta $\varepsilon$ tak dapat ditarik keluar dari operator divergensi $\nabla$ karena ia bernilai spesifik sebagai fungsi posisi $\mathbf{r}$. Nilai ini akan mengalami lompatan (diskontinuitas atau permittivity jump) eksak pada tepian tetesan dari permitivitas matriks polimer acuan ($\varepsilon_0$) menjadi permitivitas kristal cair ($\varepsilon_r \varepsilon_0$).
...
\begin{equation}
\adjustbox{max width=0.95\linewidth}{$\displaystyle \left( \varepsilon_{\text{matriks}}\frac{\partial V}{\partial n} \right)_{\text{keluar}} = \left( \varepsilon_{\text{LC}}\frac{\partial V}{\partial n} \right)_{\text{masuk}}$}
\end{equation}
...
Model tetesan berbentuk bola menuntut \textit{grid} komputasi koordinat polar bola $(r, \theta)$ pada bidang meridian. Operator nabla disesuaikan sehingga persamaan konservatif dielektrik mengambil wujud:
\begin{equation}
...
Visualisasi kontur pada Gambar \ref{fig:kasus3} (Kiri) merepresentasikan amplitudo besar medan listrik $|\mathbf{E}|$. Secara eksterior (luar), arah penyebaran fluks merambat dalam bentuk gelombang konstan linear dan merata ($E_0$). Mendekati perbatasan batas (\textit{boundary}) dengan tetesan kristal cair dielektrik (ditandai oleh garis batas putus-putus), arah perambatan vektor orientasi refraksi medan panah $\mathbf{E}$ membelok ke arah radial. Pembengkokan garis gaya medan tersebut membuktikan keberadaan intervensi orientasi (induksi polarisasi molekuler) karena perubahan permitivitas diskontinu antarmuka antara matriks polimer ke kristal cair dielektrik. Rentang area interior tetesan juga ditandai dengan perubahan kontur gradien warna biru pekat. Perbedaan tajam skalar warna ini mendemonstrasikan fenomena fisika efektivitas absorpsi redaman (\textit{depolarization} tameng) akibat susunan polarisasi induksi yang terjadi secara spasial dalam material kristal cair terkait.

Berdasarkan perbandingan irisan medan pada \texttt{E_z} sumbu sentral simetris (Gambar \ref{fig:kasus3} Kanan), dapat dilihat karakteristik ekuivalen pada perambatan medan dielektrik yang lebih homogen. Titik-titik luaran komputasi membentuk lereng horizontal lurus (konstan mendatar) murni selama merambat sejauh jarak rentang limit radius interior tetesan bola $a$. Penurunan rata-rata (atenuasi) medan di internal tetesan kristal cair ditaksir pada nilai besaran $\approx 540.0\text{ V/m}$. Nilai diskritisasi batas FVM numerik tersebut mendekati garis batas referensi kalkulasi rasio teoretik (Hukum Gauss murni) di besaran nilai ukur sebesar $\approx 555.6\text{ V/m}$. Presisi yang memuat perbedaan margin \textit{error} hanya dengan porsi toleransi nilai selisih $< 2,8\%$ membuktikan kemampuan metode algoritma komputasi interpolasi batas harmonik \textit{FVM cell-centered} mengaproksimasi lompatan fasa gelombang radial diskontinu di batas geometri lengkung (yang mana terkadang menghasilkan fluktuasi lokal kecil \textit{aliasing peak} atau \textit{spike} pada batas resolusi celah \textit{grid} komputasi).

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/kasus3.png}
\caption{\textit{Output} program Kasus 3: tetesan kristal cair PDLC \textit{ND filter} (potensial, besar dan arah medan, serta validasi medan dalam)}
\label{fig:kasus3}
\end{figure}

\vspace{2pt}
\noindent{\small Catatan: \textit{source code} lengkap Kasus 3 dapat diakses pada repositori \href{https://github.com/SandyFauzi/Fisika-Komputasi-5}{\texttt{github.com/SandyFauzi/Fisika-Komputasi-5}}.}
...
\vspace{2pt}
\noindent{\small Catatan: \textit{source code} lengkap Kasus 4 dapat diakses pada repositori \href{https://github.com/SandyFauzi/Fisika-Komputasi-5}{\texttt{github.com/SandyFauzi/Fisika-Komputasi-5}}.}
...
\vspace{2pt}
\noindent{\small Catatan: \textit{source code} lengkap Kasus 5 dapat diakses pada repositori \href{https://github.com/SandyFauzi/Fisika-Komputasi-5}{\texttt{github.com/SandyFauzi/Fisika-Komputasi-5}}.}
...
\vspace{2pt}
\noindent{\small Catatan: \textit{source code} lengkap Kasus 6 dapat diakses pada repositori \href{https://github.com/SandyFauzi/Fisika-Komputasi-5}{\texttt{github.com/SandyFauzi/Fisika-Komputasi-5}}.}
...
\subsubsection{Kegagalan Euler dan Solusi dari Skema \textit{Crank-Nicolson}}
...
\vspace{2pt}
\noindent{\small Catatan: \textit{source code} lengkap Kasus 7 dapat diakses pada repositori \href{https://github.com/SandyFauzi/Fisika-Komputasi-5}{\texttt{github.com/SandyFauzi/Fisika-Komputasi-5}}.}
